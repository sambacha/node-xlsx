...
function parse_zip(zip, opts) {
    ...
    /* GH 2017-02-27 - made this next block unconditional.
     * It appears that, otherwise, mis-mapping occurs when Chart sheets are present.
     */
...
function write_zip_type(wb, opts) {
  ...
  var z = write_zip(wb, o);
	switch(o.type) {
		case "base64": return z.generate({type:"base64", compression: opts.compression || "DEFLATE"});
		case "binary": return z.generate({type:"string", compression: opts.compression || "DEFLATE"});
		case "buffer": return z.generate({type:"nodebuffer", compression: opts.compression || "DEFLATE"});
		case "file": return _fs.writeFileSync(o.file, z.generate({type:"nodebuffer", compression: opts.compression || "DEFLATE"}));
		default: throw new Error("Unrecognized type " + o.type);
	}
}
